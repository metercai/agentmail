...
| 概念 | 说明 |
|------|------|
| **Board** | 项目看板。生命周期：active → awaiting_owner(output提交) → completed(Owner确认) |
...
| 操作 | 指令 | 发送者 | 说明 |
|------|------|--------|------|
| 创建 | `[A2A] new {项目}: {描述}` | **owner 硬编码专属** | orchestrator+verifier 必含，sender 必须为 owner |
| 更新 | `[A2A] refresh` | **owner 硬编码专属** | 增减 member、更新 role_permissions、更新 description |
