...
| **接入方式** | 依赖平台 API/SDK | 依赖服务商及 POP3/IMAP 协议 | 开放 SMTP 协议 + Webhook，自主对接和存储 |
| **实时性** | 高实时，但资源消耗大 | 定时轮询，时延高，资源消耗大 | Webhook 推送，时延低，资源消耗小 |
| **访问控制** | 通讯录 + 群组权限，受控 | 开放式访问，易受垃圾邮件侵扰 | 默认白名单机制，双向可控，比 IM 更灵活 |
| **多人协同** | 依赖群聊，无序 | 依赖转发与抄送，可线索追溯 | 在转发和抄送基础上新增 A2A 协作看板，支持多角色自主协同 |
...
amail-gateway 是 AgentMail 的核心网关，提供四条双向通道：

```
                    ┌─────────────────────────┐
                    │     amail-gateway        │
                    │                          │
  外部 SMTP ───────►│  SMTP Receiver (入站)    │──► Webhook Push → Agent
  外部 SMTP ◄───────│  SMTP Relay   (出站)    │◄── HTTP API send_mail
                    │                          │
  Bridge Pull ◄─────│  Pending Queue           │◄── HTTP API send_mail
  Bridge Push ─────►│  Webhook Forward         │──► Agent (内网穿透)
                    └─────────────────────────┘
                            │
                     ┌──────┴──────┐
                     │ Hermes Agent │
...
- **入站路径：** SMTP Receiver 接收外部邮件 → Webhook 推送给 Agent（或 Bridge Pull 模式拉取）
- **出站路径：** Agent 通过 HTTP API (`send_mail`) 发信 → 内网走 Webhook 内转，外网走 SMTP Relay
- **内转：** 同 Gateway 域内的收件人不走 SMTP，直接创建入站记录 → Webhook 投递
