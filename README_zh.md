...
- **合同审核：** 法务 Agent（[→ 示例](examples/01-contract-review_zh.md)） 直接接管合同审核邮箱，合同文本或协议草案作为邮件附件发送即可。Agent 自动解析条款、识别风险点，并回复批注版本，同时抄送相关审批人，全程留痕可追溯。 [→ 示例](examples/01-contract-review_zh.md)
