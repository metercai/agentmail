#!/usr/bin/env bash
# =============================================================================
# AmailRelay — Hermes Agent Integration Smoke Test
# =============================================================================
#
# 验证 Hermes Agent 侧与 AmailRelay 的集成是否正确。
...
#
# Usage:
#   export AMR_URL=http://46.17.41.218:38080
#   export AMR_SYS_ID=myproject
#   export AMR_ADMIN_KEY=sk-xxxx...
#   bash test/hermes_mail_relay_smoke.sh
# =============================================================================
set -euo pipefail
...
AMR_URL="${AMR_URL:-${RELAY_URL:-http://127.0.0.1:38080}}"
AMR_SYS_ID="${AMR_SYS_ID:-${SYS_ID:-}}"
AMR_ADMIN_KEY="${AMR_ADMIN_KEY:-${ADMIN_KEY:-}}"
...
api() { curl -s -H "X-Api-Key: $AMR_ADMIN_KEY" -H "Accept: application/json" "$@"; }

echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║      AmailRelay — Hermes Integration Smoke Test            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
...
if [[ -z "$AMR_ADMIN_KEY" ]]; then
    echo "  Set AMR_ADMIN_KEY (tenant_admin key) before running."
    echo "  Usage:"
    echo "    export AMR_URL=http://..."
    echo "    export AMR_SYS_ID=myproject"
    echo "    export AMR_ADMIN_KEY=sk-xxx..."
    exit 1
fi

# Test connection with whoami
WHO=$(api "$AMR_URL/api/v1/whoami" 2>/dev/null || echo '{"scope":"","tenant_id":""}')
...
echo "  ✅ Connected to $AMR_URL as tenant=$TENANT (scope=$SCOPE)"
echo ""
...
R=$(api -o /dev/null -w "%{http_code}" "$AMR_URL/api/v1/admin/tenants")
[[ "$R" = "200" ]] && pass "List tenants (200)" || fail "List tenants ($R)"

R=$(api -o /dev/null -w "%{http_code}" "$AMR_URL/api/v1/admin/tenants/$TENANT/domains")
[[ "$R" = "200" ]] && pass "List domains (200)" || fail "List domains ($R)"

DOMAINS=$(api "$AMR_URL/api/v1/admin/tenants/$TENANT/domains")
...
AGENT_RESP=$(api -X POST "$AMR_URL/api/v1/api-keys" \
...
# Agent key whoami
WHO_A=$(curl -s -H "X-Api-Key: $AGENT_KEY" "$AMR_URL/api/v1/whoami")
...
R=$(api -o /dev/null -w "%{http_code}" -X POST "$AMR_URL/api/v1/admin/whitelists" \
    -H "Content-Type: application/json" \
...
R=$(api -o /dev/null -w "%{http_code}" "$AMR_URL/api/v1/admin/whitelists?domain=mail.amail.test")
[[ "$R" = "200" ]] && pass "List whitelist (200)" || fail "List whitelist ($R)"

WL=$(api "$AMR_URL/api/v1/admin/whitelists?domain=mail.amail.test")
...
R=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$AMR_URL/api/v1/admin/whitelists" \
...
R=$(api -o /dev/null -w "%{http_code}" "$AMR_URL/api/v1/stats/tenant/$TENANT")
[[ "$R" = "200" ]] && pass "Tenant stats (200)" || fail "Tenant stats ($R)"

R=$(api -o /dev/null -w "%{http_code}" "$AMR_URL/api/v1/stats/agents?tenant_id=$TENANT")
[[ "$R" = "200" ]] && pass "Agent stats (200)" || fail "Agent stats ($R)"

# Agent self-stats
R=$(curl -s -o /dev/null -w "%{http_code}" -H "X-Api-Key: $AGENT_KEY" "$AMR_URL/api/v1/stats/agent/me")
[[ "$R" = "200" ]] && pass "Agent self-stats (200)" || fail "Agent self-stats ($R)"

# ─── Cleanup: delete test agent key ───────────────────────────────────────

if [[ -n "${AGENT_ID:-}" ]]; then
    R=$(api -o /dev/null -w "%{http_code}" -X DELETE "$AMR_URL/api/v1/api-keys/$AGENT_ID")
...
echo "  URL:        $AMR_URL"
echo "  Tenant:     $TENANT"
echo "  Admin key:  ${AMR_ADMIN_KEY:0:12}..."
