...
**一行命令完成对接。** `bash integrate.sh` 自动检测 relay、验证凭证、保存配置并运行诊断——一步到位。
...
│  外部发件人   │ ────────────→ │  amail-relay │ ─────────────────────────────────→ │ Hermes Gateway │
...
**核心设计决策：**
- Webhook secret 由 **Hermes 网关管理**（Hermes 创建，relay 接收）
...
# 10 步向导：
#   [1/10] relay 连接      — 自动检测 localhost:38080
...
| 变量 | 必填 | 默认值 | 用途 |
|---|---|---|---|
| `AMAIL_URL` | 是（auto） | — | Relay 服务器 URL |
...
1. 在 relay 上注册 `{profile_name}@{domain}` 为 webhook 路由
...
删除 profile 时，agent 的 API key 从 relay 移除，`amail.json` 删除。
...
1. 有人发送邮件到 `support.alice@admin.relay`
2. 预处理器剥离 persona 前缀，对照当前 profile 的 `agent.personalities` 配置进行校验
3. 校验通过 → agent 收到 `my_amail_addr = "support.alice@admin.relay"`，以 support 身份运行
4. 校验失败（未配置该 persona）→ 回退到 `alice@admin.relay`（记录 warning）
...
| 字段 | 来源 | 含义 |
|------|------|------|
| `message_id` | relay SMTP | 唯一标识符，用于线程追踪 |
| `subject` | relay SMTP | 邮件主题 |
| `body` | relay SMTP | 纯文本正文 |
...
| 函数 | 用途 |
|------|------|
| `setup(relay_url, system_id, admin_key?, product_code?, domain?, save_raw_snapshots?, manager_address?)` | 保存 relay 配置到 `~/.hermes/config.yaml` |
| `verify_integration(relay_url?, admin_key?)` | 运行诊断检查，返回逐项通过/失败 |
...
| 凭证 | 持有者 | 用途 |
|------|--------|------|
| Admin key (`system` scope) | Relay 运维者、`integrate.sh` | 管理域、生成激活码、配置白名单 |
| 地址激活码 | Profile 配置（磁盘） | 一次性：agent 自激活 → 返回原始 api_key |
| Agent API key (`agent` scope) | Profile 配置（磁盘） | 收发邮件、管理联系人、写入线程摘要 |
| Webhook HMAC secret | Gateway + Relay | 系统级信任：relay 与 Hermes 网关之间的签名验证 |
...
`webhook_host` 字段在集成过程中**自动探测**：通过比较 `relay_url` 与本地网卡地址，
自动确定 relay 回调 Hermes gateway 的地址：
...
使用 bridge pull 模式（无公网 IP）时，在 relay 上通过
`POST /api/v1/admin/systems/{sid}/domains` 设置 `delivery_mode: "pull"`。
SMTP 邮件存入 relay 待投递队列，不主动 POST webhook。
...
| 位置 | Key | 内容 |
|------|-----|------|
| relay `agent_state` | `profile:{address}` | 联系人画像 JSON |
| relay `agent_state` | `name:{name}` | `{"addresses": [...]}` 索引 |
| relay `agent_state` | `thread:{thread_id}` | 线程摘要文本 |
...
Relay 使用 `X-Webhook-Signature: {HMAC-SHA256(body, secret)}` 签名 webhook 载荷。
这是 Hermes webhook adapter 的通用格式。

Secret 在 Hermes 网关（生成方）和 relay（接收方，在 domain 注册时获取）之间共享。
