...
### 1.8 GATEWAY_URL 本机判断重复 + webhook_host="" 可靠性

当前代码中，`webhook_host=""` 有两种含义：

1. **GATEWAY_URL 本机** → 无 bridge 配置 → webhook_host 保持空
2. **bridge 模式 [3]** → Step 4 设 `WEBHOOK_HOST=""`，Step 5.5 自动检测后才填充

因此 `webhook_host=""` **不能**可靠表达"gateway 在本机"——bridge 模式在 Step 4 结束时也是空字符串。

**修正**：Step 4 入口先判断 `GATEWAY_URL` 是否本机。若是→写 `webhook_host=""`（明确语义）。若否→进入 3 选项，每个选项在 Step 4 内完成配置（bridge 模式自动检测也在 Step 4 内完成），写非空值。这样 `webhook_host=""` 唯一对应"本机 gateway"。
