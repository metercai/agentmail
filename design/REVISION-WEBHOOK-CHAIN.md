...
### 1.9 Bridge `hostname` 与 TLS 的耦合

当前 `push.hostname` 控制 TLS 启用（`has_tls()` = `hostname.is_some()`）。提升到顶层后，需区分：

| hostname 形式 | 示例 | TLS | agent 调用协议 |
|-------------|------|-----|-------------|
| IP:port | `"1.2.3.4:38081"` | 不需要 | `http://` |
| domain:port | `"bridge.example.com:443"` | 启用（ACME/静态证书） | `https://` |

`tls_cert`、`tls_key`、`acme_cache` 随 `hostname` 一起提升，但仅 domain 形式时生效。

### 1.10 Bridge 本地路由自动发现
...
2. **彻底清除 `bridge_url`**：所有读写代码和 legacy migration 逻辑
...
6. **Step 4 合并原 5.5**：模式选择→deployment→验证→配置写入在一个步骤内完成
7. **数据流单向**：`integrate.sh` → `amail_gateway.json` → `_auto_register_email` → `amail.json`。agent 只读 `amail_gateway.json`
8. **集成脚本可重复运行**：已配置值作为交互菜单默认值
9. **限定改动范围**：只修改文档明确列出的文件和函数路径。未提及的代码一概不动
...
`tls_cert`、`tls_key`、`acme_cache` 从 `[push]` section 随 `hostname` 一起提升到顶层，
仅 domain 形式时生效。`has_tls()` 改为 `hostname.is_some() && !is_ip_address(hostname)`。

`_auto_register_email` 调用 bridge 时的协议由 `webhook_host` 格式决定：
- 含 `.:`（域名:port） → `https://`
- 含数字 `:`（IP:port） → `http://`
...
- 已有 profile 已注册 → `_auto_register_email` 跳过（profile 的 `amail.json` 已存在）
...
Phase 1（可并行）
  └── 1a: amail-bridge
           - config.rs: `push.hostname` 提升到顶层 `hostname`
...
| 阶段 | 检测方法 |
|------|---------|
|| 1a | `cargo build --release`（bridge）；`curl POST /api/v1/routes` → `{"status":"ok","webhook_url":"..."}`；验证 IP hostname 不启用 TLS |
