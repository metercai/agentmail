...
#   [1]  relay connect   [2]  auth method     [3]  domain (skipped for product_code)
...
if [ "$LANG_CHOICE" = "zh" ]; then
    ...
    T_RELAY="配置 relay 连接"
    T_DETECT="检测到 relay 运行在"
    T_CHECKING="检测连通性..."
    T_FAILED="失败"
    T_RELAY_OK="relay 连接正常 (已运行"
    ...
    T_USE_KEY="使用此 key?"
    T_KEY_HINT="base 版 relay 启动后，admin_key 保存在 {db}.admin_key 文件中"
    ...
    T_DOMAIN_HINT="domain (如 'admin.relay'): "
    ...
    T_TEST_REG="注册测试域..."
    T_TEST_SEND="发送测试邮件..."
    T_TEST_OK="在线收发测试通过 — relay 发信正常"
    T_TEST_FAIL_SEND="发信测试失败 — 请检查 relay 配置和白名单"
    T_TEST_CLEAN="清理测试数据..."
    T_DONE="集成完成！"
    T_SUMMARY="配置摘要"
    T_AUTH_LABEL="认证方式"
    ...
    T_RELAY="Configure relay connection"
    T_DETECT="relay detected at"
    T_CHECKING="Checking connectivity..."
    T_FAILED="FAILED"
    T_RELAY_OK="relay connected (uptime"
    ...
    T_KEY_HINT="After starting base edition relay, the admin_key is saved in {db}.admin_key"
    T_KEY_PROMPT="admin_key: "
    ...
    T_DOMAIN_HINT="domain (e.g. 'admin.relay'): "
    ...
    T_TEST_OK="Send/receive test passed — relay sending works"
    T_TEST_FAIL_SEND="Send test failed — check relay config and whitelist"
...
if $AUTO_MODE; then
    RELAY_URL="${AMAIL_URL:-}"
    [ -z "$RELAY_URL" ] && step_fail "AMAIL_URL not set (required for --auto mode)"
    info "Read from AMAIL_URL: $RELAY_URL"
else
    ...
    RELAY_URL=$(ask_param "relay_url" "AMAIL_URL" "relay_url" "$DEFAULT_URL")
fi

echo -n "  $T_CHECKING "
HEALTH=$(curl -s -o /dev/null -w '%{http_code}' "$RELAY_URL/health" 2>/dev/null || echo "000")
[ "$HEALTH" != "200" ] && { echo -e "${RED}$T_FAILED (HTTP $HEALTH)${NC}"; step_fail "Cannot reach $RELAY_URL/health"; }
UPTIME=$(curl -s "$RELAY_URL/health" 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('uptime_secs','?'))" 2>/dev/null || echo "?")
echo -e "${GREEN}$T_OK${NC}"
step_ok "$T_RELAY_OK ${UPTIME}s)"
...
if $USE_PRODUCT_CODE; then
    ...
    echo -n "  $T_VERIFY "
    WHOAMI=$(curl -s "$RELAY_URL/api/v1/whoami" -H "X-Api-Key: $ADMIN_KEY" 2>/dev/null || echo '{}')
...
if ! $USE_PRODUCT_CODE; then
    ...
    if $AUTO_MODE; then
        ...
        DOMAINS_JSON=$(curl -s "$RELAY_URL/api/v1/admin/systems/$SYSTEM_ID/domains" -H "X-Api-Key: $ADMIN_KEY" 2>/dev/null || echo "[]")
...
if $AUTO_MODE; then
    ...
    if [ -z "$AMAIL_WEBHOOK_HOST" ]; then
        info "Webhook host (relay callback address, leave empty for auto-detect)"
    fi
...
# ═══════════════════════════════════════════════════════════════
# Step 5.5: Bridge deployment (remote relay only)
# ═══════════════════════════════════════════════════════════════
BRIDGE_NEEDED=false
if ! echo "$RELAY_URL" | grep -qE "127\.0\.0\.1|0\.0\.0\.0|localhost|::1"; then
    echo ""
    ...
    if [ -x "$BRIDGE_LINK" ]; then
        ...
        # ── Probe relay → agent reachability to pick push vs pull ──
        BRIDGE_MODE="push"
        echo -n "  Probing relay reachability to ${BRIDGE_ADDR}... "
        PROBE_RESULT=$(curl -s -X POST "$RELAY_URL/api/v1/admin/probe-webhook" \
...
PYEOF
    ...
    echo "$REG_OUTPUT" | while IFS=: read key val; do
        case "$key" in
            ...
            failed) info "  ⚠ $val" ;;
            no_config) info "  No relay config — skip" ;;
        esac
    done
...
result = verify_integration(relay_url="$RELAY_URL", admin_key="$ADMIN_KEY")
print(json.dumps(result, indent=2, ensure_ascii=False))
PYEOF
)
...
CREATE_RESP=$(curl -s -X POST "$RELAY_URL/api/v1/api-keys" \
    -H "X-Api-Key: $ADMIN_KEY" -H "Content-Type: application/json" \
...
if [ -n "$TEST_AGENT_KEY" ]; then
    ...
    echo -n "  $T_TEST_REG "
    DOMAIN_RESP=$(curl -s -X POST "$RELAY_URL/api/v1/admin/systems/$SYSTEM_ID/domains" \
        -H "X-Api-Key: $ADMIN_KEY" -H "Content-Type: application/json" \
    ...
    WHITELIST_RESP=$(curl -s -X POST "$RELAY_URL/api/v1/admin/whitelists" \
        -H "X-Api-Key: $ADMIN_KEY" -H "Content-Type: application/json" \
    ...
    echo -n "  $T_TEST_SEND "
    SEND_RESP=$(curl -s -X POST "$RELAY_URL/api/v1/send" \
    ...
    # Cleanup
    echo -n "  $T_TEST_CLEAN "
    [ -n "$TEST_KEY_ID" ] && curl -s -X DELETE "$RELAY_URL/api/v1/api-keys/$TEST_KEY_ID" -H "X-Api-Key: $ADMIN_KEY" > /dev/null 2>&1
    [ -n "$TEST_DOMAIN_ID" ] && curl -s -X DELETE "$RELAY_URL/api/v1/admin/system-domains/$TEST_DOMAIN_ID" -H "X-Api-Key: $ADMIN_KEY" > /dev/null 2>&1
    [ -n "$TEST_WL_ID" ] && curl -s -X DELETE "$RELAY_URL/api/v1/admin/whitelists/$TEST_WL_ID" -H "X-Api-Key: $ADMIN_KEY" > /dev/null 2>&1
    echo "$T_OK"
fi
